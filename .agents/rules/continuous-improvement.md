...
# Gemini Continuous Improvement Protocol

**Objective:** Ensure I proactively learn from tasks, capture knowledge in a structured way, **distill fundamental insights,** refine understanding, and improve efficiency and reliability. This protocol helps me maintain two key files: `memory-bank/raw_reflection_log.md` for initial detailed logging, and `memory-bank/consolidated_learnings.md` for pruned, actionable, long-term knowledge. This is vital for my optimal performance and avoiding redundant effort.
...
*   **`memory-bank/consolidated_learnings.md`**: Contains curated, synthesized, and actionable insights derived from `raw_reflection_log.md`. This is the primary, refined knowledge base for long-term use. It should be kept concise and highly relevant.

---

## 1. Mandatory Pre-Completion Reflection & Raw Knowledge Capture

Before I signal task completion (e.g., via `attempt_completion`), I **must** perform the following internal steps:

### 1.1. Task Review & Analysis:
* Review the completed task (conversation, logs, artifacts).
* **Identify Learnings:** What new information, techniques, **deeper patterns,** API behaviors, project-specific commands (e.g., test, build, run flags), environment variables, setup quirks, or successful outcomes were discovered? **What core principles can be extracted?**
* **Identify Difficulties & Mistakes (as Learning Opportunities):** What challenges were faced? Were there any errors, misunderstandings, or inefficiencies? **How can these experiences refine my future approaches (resilience & adaptation)?** Did user feedback indicate a misstep?
* **Identify Successes:** What went particularly well? What strategies, tools, or techniques were notably effective? **What were the key contributing factors?**
...
* *Example Entry in `memory-bank/raw_reflection_log.md`:*
    ...
    - Project Alpha: Specific commands.
    ---
    ```
...
This outlines refining my knowledge from `memory-bank/raw_reflection_log.md` into `memory-bank/consolidated_learnings.md`. This occurs periodically or when `raw_reflection_log.md` grows significantly, not necessarily after each task.
...
* Identify entries/parts of entries representing durable, actionable, or broadly applicable knowledge (e.g., reusable patterns, critical configurations, effective strategies, resolved errors).
...
* For identified insights:
    ...
    * Add refined knowledge to `memory-bank/consolidated_learnings.md`, organizing logically (by topic, project, or tech) for easy retrieval.
...
These guidelines apply to entries in my `memory-bank/raw_reflection_log.md` (initial capture) and especially to my `memory-bank/consolidated_learnings.md` (my refined, long-term knowledge).

* **Prioritize High-Value Insights:** Focus on lessons that significantly impact my future performance, **lead to more robust or generalizable understanding,** or detail critical errors and their resolutions, major time-saving discoveries, fundamental shifts in understanding, and essential project-specific configurations.
...
* **Support Continuous Improvement:** The ultimate goal is to avoid repeating mistakes, accelerate future tasks, and make my operations more robust and reliable. Frame all knowledge with this in mind.
