...
current_position: "v0.0.2.3 (Build 16) — Phase 12: Security Suite, Panic Purge, Audit Logging & Web Server v0.0.2.3 Sync Parity; next Phase 13: Advanced Import/Export, Bitwarden Migration & Google Authenticator Multi-QR (v0.0.3.0)"
statistics:
...
> 🌐 **Web Server v0.0.2.3 Alignment Note (2026-09-22)**:
> ShellGuard Web Server released **v0.0.2.3 (Build 25)** ("The Deep Ingestion & Vault Parity Molt") introducing dynamic RFC 6238 TOTP URI formatting (`totpUtils.formatTotpSecret`) and Bitwarden ingestion. Phase 12 weaves in **Task 24b** to align Android's downstream sync engine (`TotpRepository.kt`) with `TotpUriParser`, ensuring `otpauth://` URIs with non-default algorithms/digits/periods are parsed seamlessly rather than raw-injected into Base32 decoding.
...
> Phase Feature Set Overview:
> Delivers advanced vault security controls (tap-to-reveal tokens, screen security toggle, emergency panic purge trigger integration), a local append-only security Audit Log recording important cryptographic and access events, and complete protocol re-alignment with ShellGuard Web Server v0.0.2.3 (Build 25) restoring dynamic TOTP URI parsing during remote synchronization.

- [x]  Task 23: [Functionality] Security Preference Controller, Panic Trigger Handler & Room Audit Log DAO
...
- [x]  Task 24: [UI Component] Security Sub-screen (Tap-to-Reveal, Screen Security, Panic Purge) & Audit Log Screen
...
- [x]  Task 24b: [Sync & Parity Fix] Web Server v0.0.2.3 Sync Re-alignment & Dynamic TOTP URI Engine

Description: Re-align the remote sync intake pipeline in `TotpRepository.kt` with the Web Server v0.0.2.3 specification:
- **Dynamic TOTP Ingestion**: In `syncRemoteVault`, pass decrypted `pearl.totp_secret` through `TotpUriParser.parse()`:
  - If the secret is formatted as an `otpauth://totp/...` or `steam://` URI (generated by Web Server v0.0.2.3 `formatTotpSecret` for non-default parameters or Bitwarden imports), extract the normalized Base32 `secret` and preserve dynamic `algorithm` (`SHA1`, `SHA256`, `SHA512`, `STEAM`), `digits` (`5`, `6`, `8`), and `period` (`15`, `30`, `60`).
  - Fall back to clean Base32 with defaults (`SHA1`, `6`, `30`) if raw Base32 was returned.
  - Fall back to URI label/issuer for `title` and `username` if server pearl metadata is blank.
- **Delta Sync Optimization for Server Null `updated_at`**:
  - The Web Server SQLite `vault_pearls` table does not carry an `updated_at` column (returns `null`).
  - In `classifyDeltaPearls`, add a content-based fast comparison comparing local Room entity `(secret, algorithm, digits, period, title, username, category)` against the freshly decrypted candidate to avoid unnecessary Room database write transactions when items are identical.
- **Test Oracle Coverage**:
  - Implement comprehensive unit tests in `TotpRepositoryTest.kt` verifying that `syncRemoteVault`:
    1. Correctly decodes and syncs raw Base32 pearls.
    2. Correctly decodes and syncs `otpauth://totp/` URIs with SHA256 / 8 digits.
    3. Correctly decodes and syncs `otpauth://totp/` URIs with custom periods (60s).
    4. Accurately updates `TotpItemEntity` attributes in Room without token calculation failure.

> Success Criteria: Remote codes with non-default algorithms, 8 digits, or custom periods synced from Web Server v0.0.2.3 generate valid rolling TOTP tokens on device without crashes or invalid Base32 decoding errors.

---

## Phase 13: Advanced Import/Export, Bitwarden Migration & Google Authenticator Multi-QR [v0.0.3.0 (Build 17) — Milestone 3]
...
## Phase 14: Home Screen Interactive Glance Widgets & Icon Pack Manager [v0.1.0.0 (Build 18) — Open Beta Candidate]
...
## Phase 15: ClawKey Vault Creation, Import Authentication & Duplicate Resolution [v0.1.1.0 (Build 19)]
